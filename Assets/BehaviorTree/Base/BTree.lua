...
function BT.BTree:New(gameObject, name)
    ...
    o.tParentIndex = {}           --val is int -可以找到其父亲的index
    o.tChildrenIndex = {}         --val is {int} 找到某节点的所有孩子
    o.tRelativeChildIndex = { 1 } --val is int 该task在其父亲中处于第几个
    o.tParentCompositeIndex = {}  -- val is int -可以找到是Composite类型且离自己最近的祖先的index，用于中断评估
